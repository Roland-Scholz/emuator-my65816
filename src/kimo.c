...
static char* diskName = ".\\res\\SD_32MB.dat";
...
void loadDiskImage() {
	...
	if (f != NULL) {
		printf("%s disk read: %d\n", diskName, disksize = fread(disk, 1, DISKSIZE, f));
		fflush(stdout);
		fclose(f);
	}
}
...
static void saveDiskImage() {
	...
	if (disk != NULL) {
		...
		if (f != NULL) {
			printf("%s disk write: %d\n", diskName, fwrite(disk, 1, disksize, f));
			fflush(stdout);
			fclose(f);
		}
	}
